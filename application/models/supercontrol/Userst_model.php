...
function users_view_join($id){
		$this->db->select('user_registration.*,countries.nicename');
		$this->db->from('user_registration');
		$this->db->join('countries', 'countries.id = user_registration.country', 'INNER'); 
...
function users_edit($id, $datalist){
		...
		$this->db->where('uid', $id);
		$this->db->update('user_registration',$datalist);
	}
...
function updt($stat,$id){
		...
		$sql ="update user_registration set user_status=$stat where uid=$id ";
		$query = $this->db->query($sql);
		//return($query->num_rows() > 0) ? $query->result(): NULL;
	}
