...
function show_video(){
	$this->db->select('*');
	$this->db->from('video');
	$this->db->join('video_category', 'video_category.category_id = video.category_id');
	$query = $this->db->get();
	$result = $query->result();
	return $result;
		/*$this->db->select('*');
...
function updt($stat,$id){
		...
		$sql ="update video set status=$stat where restaurant_id=$id ";
