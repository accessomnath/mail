...
public function insert_property($data) {
	...
	function show_property_multi_image($id){
		$sql ="select * from multi_img where property_id = '".$id."'";
...
function show_property()
	...
	function show_propertyjoin()
	{
		...
		$this->db->join('property_type', 'property_type.property_type_id = property.pid'); 
		$query = $this->db->get();
		$result = $query->result();
		return $result;
	}
...
function updt($stat,$id){
		...
		$sql ="update property set property_status=$stat where id=$id ";
		$query = $this->db->query($sql);
		//return($query->num_rows() > 0) ? $query->result(): NULL;
	}
	...
	$this->db->select ( '*' ); 
	$this->db->from('property');
	$this->db->join('property_type', 'property.pid = property_type.property_type_id');
	//$this->db->join('multi_img', 'multi_img.property_id = property.id');
	$this->db->where('property.id', $id);

/*$this->db->select('property.*,category.*,multi_img.*');    
$this->db->from('property');
$this->db->join('category', 'property.pid = category.category_id');
$this->db->join('multi_img', 'property.id = multi_img.property_id');
$this->db->where('property.id', $id);*/
$query = $this->db->get();
//$query = $this->db->get();
return $query->result();
